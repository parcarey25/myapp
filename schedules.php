...
/* --------------------------------------------------------------------------
   Ensure bookings table exists (first-time install safety)
   NOTE: If the table already exists, this does nothing. To add new columns
   to an existing table you must run ALTER TABLE manually (we already did
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    // Get member id_number from users table (for the receipt view)
    $idNumber = '';
    if ($uid) {
    ...
    $booking_id = null;
    if (!$errors) {
        // Insert booking. `status` column defaults to 'pending'.
        if ($st = $conn->prepare("
        ...
        ")) {
            ...
            if ($st->execute()) {
                ...
                $errors[] = 'Could not save booking (DB error).';
            }
            $st->close();
        } else {
    ...
    <div class="container my-4">
        <div class="card p-4 mx-auto" style="max-width:780px">
            ...
            <?php else: ?>
                ...
                <p class="text-muted">Thanks! Here are your details (status: Pending trainer approval).</p>
                ...
                <div class="table-responsive">
                    <table class="table table-dark table-sm">
                        ...
                        <tr>
                            <th>ID Number</th>
                            <td><?= clean($idNumber !== '' ? $idNumber : 'Not set') ?></td>
                        </tr>
                        <tr>
                            <th>Facility</th>
                            <td><?= clean($facility_name) ?> <span class="text-muted">(<?= clean($facility_slug) ?>)</span></td>
                        </tr>
                        ...
                        <?php if ($notes !== ''): ?>
                            <tr><th>Notes</th><td><?= nl2br(clean($notes)) ?></td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
...
// If GET action=edit: render edit form
if ($action === 'edit' && $id > 0) {
    $row = load_booking($conn, $id);
    if (!$row || !can_manage($row, $role, $uid, $uemail)) {
        http_response_code(403);
...
$rows = [];
if ($isAdmin) {
    $sql = "SELECT id, facility_name, facility_slug, date, time,
                   full_name, email, status, notes, user_id, created_at
            FROM bookings WHERE 1=1";
    $params = [];
    $types  = '';

    if ($filter_fac !== '') {
        $sql .= " AND facility_slug = ?";
        $params[] = $filter_fac;
        $types   .= 's';
    }
    if ($filter_date !== '') {
    ...
    if ($uid) {
        $st = $conn->prepare("
            SELECT id, facility_name, facility_slug, date, time,
                   full_name, email, status, notes, user_id, created_at
            FROM bookings
            WHERE user_id = ?
...
<div class="container my-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <?php if (!$isAdmin): ?>
            <small class="muted">Showing bookings for your account.</small>
        <?php endif; ?>
    </div>
        ...
        <form method="get" class="card p-3 mb-3 filter-bar">
            <div class="form-row">
                <div class="col-md-5">
                    ...
                    <input class="form-control" name="facility"
                           value="<?= clean($filter_fac) ?>" placeholder="e.g., boxing, muay-thai">
                </div>
                <div class="col-md-5">
                ...
                <table class="table table-dark table-striped table-hover table-sm mb-0">
                    ...
                    <?php foreach ($rows as $r): ?>
                        <tr>
                            <td><?= (int)$r['id'] ?></td>
                            <td>
                                <?= clean($r['facility_name']) ?>
                                <span class="text-muted">(<?= clean($r['facility_slug']) ?>)</span>
                            </td>
                            <td><?= clean($r['date']) ?></td>
                            <td><?= clean($r['time']) ?></td>
                            ...
                            <td class="text-right actions">
                                <a class="btn btn-ghost btn-sm"
                                   href="schedules.php?action=edit&id=<?= (int)$r['id'] ?>">Edit</a>
                                <form method="post"
                                      action="schedules.php?action=delete&id=<?= (int)$r['id'] ?>"
                                      onsubmit="return confirm('Delete booking #<?= (int)$r['id'] ?>?');">
                                    <input type="hidden" name="csrf" value="<?= $CSRF ?>">
