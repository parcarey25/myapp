<<<<<<< HEAD
<?php
// pending_users.php — Staff/Admin: review + approve/reject pending accounts
session_start();
...
// If you don't have a status column, show instructions instead of a broken page
if (!$has_status) {
  ?>
  <!doctype html><html><head>
  <meta charset="utf-8"><title>Pending Accounts</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@4.5.2/dist/css/bootstrap.min.css" rel="stylesheet">
  </head><body class="bg-dark text-white">
  <div class="container py-5">
    <div class="alert alert-warning">
      <h4 class="mb-2">Missing <code>users.status</code> column</h4>
      <p>Run this once in your database:</p>
      <pre class="mb-0">ALTER TABLE users ADD COLUMN status ENUM('pending','active','rejected') NOT NULL DEFAULT 'pending';</pre>
    </div>
    ...
    if ($uid <= 0) {
      ...
      if ($act === 'reject') {
        if ($st = $conn->prepare("UPDATE users SET status='rejected' WHERE id=?")) {
          $st->bind_param('i', $uid);
          $flash = $st->execute() ? 'User rejected.' : 'Failed to reject user.';
          $st->close();
        }
      }
      if ($act === 'set_role') {
        $new = $_POST['new_role'] ?? '';
        if (!in_array($new, ['member','trainer','staff','admin'], true)) {
          $errors[] = 'Invalid role.';
        }
        if ($new==='admin' && $myRole!=='admin') {
          $errors[] = 'Only admins can promote to admin.';
        }
      ...
      if ($act === 'set_expiry' && $has_expires) {
        ...
        if (!preg_match('~^\d{4}-\d{2}-\d{2}$~',$date)) {
          $errors[] = 'Invalid date (YYYY-MM-DD).';
        } else {
          $date_full = $date.' 23:59:59';
          if ($st=$conn->prepare("UPDATE users SET membership_expires_at=? WHERE id=?")) {
            $st->bind_param('si', $date_full, $uid);
            $flash = $st->execute() ? 'Expiry set.' : 'Failed to set expiry.';
            $st->close();
          }
        }
      }
...
/* ---------- filters & search ---------- */
$q      = trim($_GET['q'] ?? '');
$roleF  = trim($_GET['role'] ?? '');  // optional role filter among pending
...
$sql = "SELECT $cols FROM users WHERE ".implode(' AND ', $where)." ORDER BY id DESC LIMIT 500";
...
<head>
<meta charset="utf-8">
<title>Pending Accounts | RJL Fitness</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@4.5.2/dist/css/bootstrap.min.css" rel="stylesheet">
<style>
...
<nav class="navbar navbar-dark px-3">
  ...
  <div class="ml-auto">
    <a class="btn btn-outline-light btn-sm" href="users.php">All Users</a>
    <a class="btn btn-outline-light btn-sm" href="home.php">Dashboard</a>
  </div>
</nav>

<div class="container py-4">
  <div class="card p-3 mb-3">
    <h4 class="mb-3">⏳ Pending Accounts</h4>
      ...
      <div class="form-group col-md-4">
        ...
        <select class="custom-select" name="role">
          ...
          <?php foreach ($roles as $r): ?>
            <option value="<?= h($r) ?>" <?= $roleF===$r?'selected':''; ?>><?= ucfirst($r) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <table class="table table-dark table-striped table-sm mb-0">
        <thead>
          <tr>
            ...
            <th>Email</th>
            <th>Role</th>
            <?php if ($has_expires): ?><th>Set Expiry</th><?php endif; ?>
        ...
        <tbody>
          <?php if (!$rows): ?>
            <tr><td colspan="8" class="text-center text-muted">No pending accounts.</td></tr>
          <?php else: foreach ($rows as $u): ?>
            <tr>
              <td><?= (int)$u['id'] ?></td>
              <td>
                <?php $avatar = ($has_avatar && !empty($u['avatar_path'])) ? $u['avatar_path'] : 'photo/logo.jpg'; ?>
                  ...
                  <?php if ($has_valid_path): ?>
                    <div class="mt-1">
                      <?php if (!empty($u['valid_id_path'])): ?>
                        <a href="<?= h($u['valid_id_path']) ?>" target="_blank">View file</a>
                      <?php else: ?>
              ...
              <td>
                ...
                <form method="post" class="d-inline ml-1" onsubmit="return confirm('Reject this user?');">
                  <input type="hidden" name="csrf" value="<?= h($CSRF) ?>">
                  <input type="hidden" name="user_id" value="<?= (int)$u['id'] ?>">
