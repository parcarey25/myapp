<<<<<<< HEAD
<?php
// home_member.php — Member dashboard showing profile avatar + quick links
if (session_status()===PHP_SESSION_NONE) session_start();
if (!isset($_SESSION['user_id'])) { header('Location: login.php'); exit; }
require __DIR__.'/db.php';

$userId=(int)$_SESSION['user_id'];
// Load current user (includes avatar_path)
$user=[
  'username'=>$_SESSION['username']??'',
  'email'=>'','full_name'=>'','id_number'=>'',
  'valid_id_path'=>'','valid_id_status'=>'none',
  'membership_expires'=>null,'avatar_path'=>''
];

if($st=$conn->prepare("SELECT email, full_name, id_number, valid_id_path, valid_id_status, membership_expires_at, avatar_path FROM users WHERE id=? LIMIT 1")){
  ...
  if($rs=$st->get_result()){
    if($row=$rs->fetch_assoc()){
      ...
      $user['membership_expires']=$row['membership_expires_at']??null;
...
if($r=$conn->query("SELECT COUNT(*) c FROM users WHERE role='member'")){ $totalMembers=(int)$r->fetch_assoc()['c']; $r->free();}
if($r=$conn->query("SELECT COUNT(*) c FROM users WHERE role='trainer'")){ $totalTrainers=(int)$r->fetch_assoc()['c']; $r->free();}
?>
<!doctype html>
<html lang="en">
<head>
...
.navbar{background:linear-gradient(90deg,#000,var(--brand));position:relative;z-index:1}
  ...
  <div class="ml-auto d-flex align-items-center">
    ...
    <div class="profile-wrap" id="user-info">
      ...
      <div id="profilePanel" class="profile-panel" role="dialog" aria-hidden="true">
        ...
        <div class="panel-row">
          <span>Valid ID</span>
          <span><?php if($user['valid_id_path']):?><a target="_blank" href="<?=htmlspecialchars($user['valid_id_path'])?>">View</a><?php else:?>Not uploaded<?php endif;?></span>
        </div>
        <hr>
        <a href="upload_avatar.php" class="btn btn-outline-light btn-block mb-2">Change Profile Picture</a>
  ...
  <div class="dashboard">
    ...
    <div class="row mt-3">
      <div class="col-md-4">
        ...
        <a class="side-link" href="#user-info" id="openUserInfo">🧑 User Info (Valid ID)</a>
        <a class="side-link" href="attendance.php">🕒 Attendance</a>
        <a class="side-link" href="schedules.php">🗓 My Reservation Slot / Booking</a>
        <a class="side-link" href="membership.php">💠 Membership Status</a>
      </div>
      <div class="col-md-8">
        <div class="p-3" style="background:#222;border-radius:12px;border-left:4px solid #ff1a1a">
          <h5 class="text-danger">Membership</h5>
          <p class="mb-1"><strong>Expires on:</strong> <?=htmlspecialchars(fmtDate($user['membership_expires']))?></p>
          <div class="d-flex" style="gap:10px;flex-wrap:wrap">
            <a class="btn btn-danger" href="membership.php">Extend Membership</a>
            <a class="btn btn-outline-light" href="payments.php">Go to Payments</a>
          </div>
        </div>

        <div class="p-3 mt-3" style="background:#222;border-radius:12px;border-left:4px solid #444">
          <h6 class="text-muted mb-2">Quick Stats</h6>
          <p class="mb-1">Total Members: <?=$totalMembers?></p>
          <p class="mb-0">Total Trainers: <?=$totalTrainers?></p>
        </div>
      </div>
    </div>
  </div>
...
(function(){
  ...
  const openUserInfo=document.getElementById('openUserInfo');
  ...
  if(openUserInfo){ openUserInfo.addEventListener('click',(e)=>{ e.preventDefault(); e.stopPropagation(); openPanel(); window.scrollTo({top:0,behavior:'smooth'}); }); }
  if(profilePanel){ profilePanel.addEventListener('click',(e)=>{ e.stopPropagation(); }); }
  document.addEventListener('click',()=>{ if(profilePanel && profilePanel.classList.contains('show')) closePanel(); });
