...
// Set timezone (so time matches PH time)
...
// Handle POST actions (Reset row / Save all)
    ...
    if (isset($_POST['reset_attendance_id'])) {
        ...
        if ($attId > 0) {
            if ($st = $conn->prepare("UPDATE attendance SET time_in = NULL, time_out = NULL WHERE id = ?")) {
                ...
                $st->execute();
                $st->close();
                $flash = 'Time in/out cleared for this record.';
            }
        }
    } elseif (isset($_POST['save_all'])) {
        // All swipes are already saved directly to DB by rfid_attendance.php
        $flash = 'All attendance for ' . $selectedDate . ' is saved.';
    }

    // Redirect to avoid resubmission
    $qs = http_build_query(['date' => $selectedDate, 'q' => $search]);
...
.burger-btn{
  width:40px;height:32px;border-radius:10px;border:1px solid #333;
  background:#111;display:flex;flex-direction:column;justify-content:center;
  padding:4px;cursor:pointer;margin-right:10px;
}
.burger-line{
  height:2px;background:#fff;border-radius:999px;margin:3px 0;
}
.profile-wrap{position:relative}
.profile-circle{
  width:42px;height:42px;border-radius:50%;overflow:hidden;
  border:2px solid var(--brand-soft);background:#222;cursor:pointer
}
...
.profile-panel{
  ...
  box-shadow:0 16px 40px rgba(0,0,0,.6);padding:14px;display:none;z-index:3000;
}
...
.sidebar{
  position:fixed;top:56px;left:0;bottom:0;width:230px;
  background:var(--panel);border-right:1px solid var(--line);
  padding:10px;transform:translateX(-100%);transition:transform .2s ease-out;
  z-index:900;
}
.sidebar.show{transform:translateX(0);}
.side-link{
  display:block;padding:9px 10px;margin:4px 0;border-radius:10px;
  background:#161616;border:1px solid #262626;color:#eee;text-decoration:none;
  font-size:.9rem;
}
.side-link:hover{background:#202020;text-decoration:none;color:#fff}
.main-wrap{padding:16px;}
@media (min-width: 992px){
  .sidebar{position:static;transform:none;width:230px;}
  .layout-row{display:flex;}
  .main-wrap{flex:1;padding:16px 16px 16px 8px;}
}
.card-panel{
  background:var(--panel);border-radius:14px;border:1px solid var(--line);padding:18px;
}
.badge-soft{
...
.table td{border-top:1px solid #222;font-size:.85rem;vertical-align:middle;}
.table-dark{background:#101010;color:#eee;}
.table-dark tbody tr:hover{background:#181818;}
.btn-danger{background:var(--brand);border:none;}
.btn-danger:hover{background:#e60000;}
.btn-outline-light{border-color:var(--brand-soft);color:#fff;}
.btn-outline-light:hover{background:var(--brand-soft);}
.filter-label{font-size:.8rem;color:var(--muted);margin-bottom:2px;}
...
<nav class="navbar navbar-dark px-3">
  <div class="d-flex align-items-center">
    <button class="burger-btn" id="burgerBtn" type="button" aria-label="Toggle menu">
  ...
  <div class="ml-auto d-flex align-items-center">
    ...
    <div class="profile-wrap" id="user-info">
      ...
      <div id="profilePanel" class="profile-panel" role="dialog" aria-hidden="true">
        ...
        <div class="panel-row"><span>Valid ID Status</span><span><?= htmlspecialchars(strtoupper($user['valid_id_status'] ?: 'NONE')) ?></span></div>
        <div class="panel-row">
          <span>Valid ID</span>
          <span>
...
<div class="layout-row">
  ...
  <div class="sidebar" id="sidebar">
    <a class="side-link" href="home_staff.php">🏠 Staff Dashboard</a>
    <a class="side-link" href="all_attendance.php">📅 All Attendance</a>
    <a class="side-link" href="users.php">👥 Users</a>
    <a class="side-link" href="pending_users.php">🕒 Pending Accounts</a>
    <a class="side-link" href="pos.php">🧾 POS</a>
    <a class="side-link" href="rfid_load.php">💳 Load RFID Wallet</a>
    <a class="side-link" href="extend_membership.php">💰 Membership Payment</a>
  </div>
    ...
    <div class="card-panel">
      ...
      <div class="table-responsive">
        <table class="table table-dark table-sm mb-2">
          ...
          <tbody>
          <?php if (!$rows): ?>
            <tr><td colspan="8" class="text-center small-muted">No users found.</td></tr>
          <?php else: ?>
...
<script>
// burger + profile dropdown + RFID JS
(function(){
  ...
  // RFID modal behaviour
  $('#rfidModal').on('shown.bs.modal', function(){
    if (rfidInput) {
      rfidInput.value = '';
      rfidInput.focus();
      if (rfidStatus) rfidStatus.textContent = '';
    }
  });
