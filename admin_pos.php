...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if ($identifier==='') {
    ...
    if($stmt->fetch()){
      ...
      $staff_id=(int)$_SESSION['user_id']; // admin id as staff_id
      ...
      $u=$conn->prepare("UPDATE users SET membership_expires_at=? WHERE id=?"); $u->bind_param('si',$new_exp,$uid); $u->execute(); $u->close();

      $msg = 'Payment saved. New expiry: '.date('M d, Y g:ia', strtotime($new_exp));
    } else { $msg='User not found or not active.'; }
  }
}
?>
...
<div class="container py-4">
  ...
  <form method="post" class="card p-3">
    ...
    <div class="form-row">
      <div class="form-group col-md-3"><label>Months</label><input type="number" name="months" class="form-control" value="1" min="1"></div>
      <div class="form-group col-md-3"><label>Amount</label><input type="number" step="0.01" name="amount" class="form-control" value="0"></div>
      ...
      <div class="form-group col-md-3"><label>Reference (optional)</label><input name="reference" class="form-control"></div>
    </div>
