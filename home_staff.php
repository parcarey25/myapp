...
if (!in_array(strtolower($_SESSION['role']??''), ['staff','admin'], true)) { header('Location: home.php'); exit; }
...
// badges
$pendingUsers=0;$pendingIDs=0;
if($r=$conn->query("SELECT COUNT(*) c FROM users WHERE status='pending'")){$pendingUsers=(int)$r->fetch_assoc()['c'];$r->free();}
if($r=$conn->query("SELECT COUNT(*) c FROM users WHERE valid_id_status='pending'")){$pendingIDs=(int)$r->fetch_assoc()['c'];$r->free();}
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <div class="dashboard">
    ...
    <div class="row mt-3">
      <div class="col-md-4">
        ...
        <a class="side-link" href="users.php">📚 All User Info</a>
        <a class="side-link" href="pending_users.php">🕒 Pending Accepting</a>
        <a class="side-link" href="pos.php">🧾 POS (Daily/Weekly/Monthly)</a>
        <a class="side-link" href="admin_dashboard.php">📈 Admin Dashboard</a>
        <a class="side-link" href="extend_membership.php">PAYMENT</a>
        <!-- UPDATED: staff load wallet using RFID -->
        <a class="side-link" href="rfid_load.php">LOAD ACCOUNT (RFID)</a>
      </div>
      <div class="col-md-8">
        <div class="p-3" style="background:#222;border-radius:12px;border-left:4px solid #ff1a1a">
          <h5 class="text-danger">Quick Status</h5>
          <p class="mb-1">Pending Users: <span class="badge-soft"><?=$pendingUsers?></span></p>
          <p class="mb-1">Pending ID Verifications: <span class="badge-soft"><?=$pendingIDs?></span></p>
          <div class="mt-2">
            <a class="btn btn-danger btn-sm" href="pos.php">Open POS</a>
            <a class="btn btn-outline-light btn-sm" href="pending_users.php">Review Pending</a>
            <a class="btn btn-outline-light btn-sm" href="users.php">Browse Users</a>
          </div>
        </div>
      </div>
    </div>
